...
export const pageCanvasProtocol: PageProtocolDefinition = {
  ...
  aiHints: [
    ...
    '页面脚本直接使用全局对象 vars、tags、components、message、change、page、node，不要生成 Ctx.xxx。',
    ...
    '系统变量/后端点位在普通脚本中使用 tags.read/tags.write/tags.subscribe，ScadaBridge 只用于 customHtml。',
    '脚本调用其他组件物料专属方法使用 components.call(componentIdOrName, methodName, ...args)。',
    '通用运行态控制优先使用 components.setStyle/setProps/show/hide/enable/disable，适合按钮变色、组件显隐和禁用启用。',
  ],
};
